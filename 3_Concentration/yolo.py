...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "ml_model_path"     : "calibration_model.pkl",
        "model_path"        : "detection_model.pth",
        "classes_path"      : "model_data/voc_classes_Fe-S.txt",
        #---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
        #---------------------------------------------------------------------#
        "input_shape"       : [640, 640],
    ...
    def generate(self, onnx=False):
        ...
        if os.path.exists(self.ml_model_path):
            import joblib
            print(f"[INFO] Loading ML regression model: {self.ml_model_path}")
            self.ml_model = joblib.load(self.ml_model_path)
        else:
            print(f"[WARN] ML model not found: {self.ml_model_path}, detection class only")
